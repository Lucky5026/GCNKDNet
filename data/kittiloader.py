...
def read_pose(filename, Tr):
    ...
    '''
    Tr_inv = np.linalg.inv(Tr)#4x4
    Tlist = []
    poses = np.loadtxt(filename, dtype=np.float32)
    for i in range(poses.shape[0]):
        one_pose = poses[i,:]#12x1
...
def points_sample(points, sn, npoints):
    ...
    choice_idx = np.random.choice(size,npoints, replace=False)  #
    new_points=points[choice_idx,:]
    new_sn=sn[choice_idx,:]
...
def get_pointcloud(filename, npoints):
    ...
    pc, sn = points_sample(pc, sn, npoints)#16384x3,16384x4
...
class KittiDataset(data.Dataset):
    def __init__(self, root, seq, npoints):
        ...
        Tr = read_calib(self.calib_path)#4x4
        self.Tlist = read_pose(self.poses_path, Tr)
        self.dataset = self.make_dataset()
    
    def __getitem__(self, index):
        src_idx, dst_idx = self.dataset[index] # 27,37
        ...
        src_T = self.Tlist[src_idx]# 4x4
        dst_T = self.Tlist[dst_idx]#4x4
        relaT = np.linalg.inv(dst_T).dot(src_T)#4x4
